...
def UploadAction(wavs):
    ...
    try:
        ...
        m_Zip = os.path.basename(canZip)
        old_canZip = os.rename(canZip, canZip.replace(m_Zip, "old_canZip.zip"))
        os.system(
            f"mfa train {tempFolder} {canDict} {old_canZip} {canZip} -j8 --output_directory {outputFolder}")
        def mvRename(p):
